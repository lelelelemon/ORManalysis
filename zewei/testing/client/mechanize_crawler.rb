...
class Crawl 
  def initialize(host, login, user, password)
    @host = host
    @agent = Mechanize.new()
    @agent.add_auth(host + login, user, password)
  end
  ...
  def handle_page(page)
    ...
    page.links.each do |link|
      next if @agent.visited_page(link.uri)
  ...
  def send_requests(tags)
    tags.each do |tag|
      next if @agent.visited_page(@host + tag)
      page = @agent.get(@host + tag)
...
crawl_task = Crawl.new("http://localhost:3000", "/login", "test", "test")
#crawl_task = Crawl.new("http://www.cs.uchicago.edu", "/login", "test", "test")
