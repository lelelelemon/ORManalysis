...
t(:filesize, :scope => [:media, :files, :edit])
 ...
 if params[:common] 
 t(:alt, :scope => [:browser,:image]) 
 params[:alt] 
 t(:align, :scope => [:browser,:image]) 
 selected_class = params[:class].blank? ? nil : params[:class] 
 [nil, "left","right","block"].each do |al| 
 al 
 'selected="selected"' if al == selected_class 
 t(al || "-", :scope => [:browser,:image,:alignments]) 
 end 
 t(:dimensions, :scope => [:browser,:image]) 
 params[:width] 
 params[:height] 
 end 
 ...
 t(:dialog_title, :scope => [:browser,:image]) 
