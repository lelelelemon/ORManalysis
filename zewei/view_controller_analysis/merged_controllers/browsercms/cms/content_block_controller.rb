...
ruby_code_from_view.ruby_code_from_view do |rb_from_view|
 ...
 h "#{@exception.class.name}: #{@exception.message}" 
 
   other_version.version - (other_version.lock_version - your_version.lock_version) 
 other_version.updated_by.full_name 
 ...
 yield :sidebar_after if content_for(:sidebar_after)
  ...
  other_version.version 
 other_version.attributes.to_a.reject{|e| ['id', 'deleted','version','lock_version','created_at','updated_by_id','updated_at','version_comment', 'published','hidden','archived'].include?(e.first) }.sort_by(&:first).each do |field, value| 
h field 
h value 
 end 
