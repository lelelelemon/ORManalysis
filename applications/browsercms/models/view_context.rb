  ...
  # 1. Need to add all helpers from that the ContentController has access to:
  # 2. Add helpers for the portlet or content block
  #
  # Understanding how ContentController renders a template:
  ...
  class ViewContext < ActionView::Base
    ...
    def initialize(controller, attributes_to_assign)
      ...
      self.class.send(:include, Engine.routes.url_helpers)

      # Need to add PageRoute helpers to the view
...
#      Rails.logger.warn "Assigned these variables: #{attributes_to_assign}"
    ...
    # Returns the routes for the Engine for view that need to access them.
    def cms
