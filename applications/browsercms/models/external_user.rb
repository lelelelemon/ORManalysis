  ...
  #   user = ExternalUser.authenticate('stan.marsh', 'southpark-crm')
  #   user.authorize('cms-admin')
  # end
  # ```
  class ExternalUser < PersistentUser
    ...
    class << self
      ...
      # @return [ExternalUser] An ExternalUser record which has been persisted in the database.
      def authenticate(login, source, info={})
        info = work_around_rails_4_serialization_bug(info)
        criteria = {login: login, source: source}
        existing = ExternalUser.where(criteria).first
        if existing
          existing.update(info)
          return existing
        end
        criteria.merge!(info)
        new_user = ExternalUser.create!(criteria)
        new_user.groups << Group.guest if Group.guest
        new_user
      end
    ...
    def authorize(*group_codes)
      new_groups = group_codes.collect { |code| Group.with_code(code).first }
