  ...
  class ContentType
    ...
    DEFAULT_CONTENT_TYPE_NAME = 'HtmlBlock'

    class << self
      def named(name)
        [ContentType.new(name: name)]
      end
      ...
      #
      # @return [Hash<Symbol, ContentType]
      ...
      #
      # @return [Array<ContentType] An alphabetical list of content types.
      def available
        subclasses = ObjectSpace.each_object(::Class).select do |klass|
          klass < Concerns::HasContentType::InstanceMethods
        end
        subclasses << Portlet
        subclasses.uniq! { |k| k.name } # filter duplicate classes
        subclasses.map do |klass|
          unless klass < Portlet
            ContentType.new(name: klass.name)
          end
      ...
      def default()
        ContentType.new(name: DEFAULT_CONTENT_TYPE_NAME)
      end

      # Returns only user generated Content Blocks
      def user_generated_connectables()
        available.select { |content_type| !content_type.name.starts_with?("") }
      end
    ...
    # Given a 'key' like 'html_blocks' or 'portlet'. Looks first for a class in the  namespace, then again without it.
    ...
    def self.find_by_key(key)
      ...
      unless klass
        raise "Couldn't find ContentType for '#{key}'. Checked for classes #{class_name} and #{class_name}."
      end
      klass.content_type
    end
    ...
    # @deprecated
    def save!
      ActiveSupport::Deprecation.warn "ContentType#save! should no longer be called. Content Types do not need to be registered in the database."
    end

    def self.create!
      ActiveSupport::Deprecation.warn "ContentType.create! should no longer be called. Content Types do not need to be registered in the database."
    end
    ...
    def display_name
      model_class.respond_to?(:display_name) ? model_class.display_name : Behaviors::Connecting.default_naming_for(model_class)
    end
    ...
    def content_block_type
      n = name.starts_with?("") ? name.demodulize : name
