...
class Group < ActiveRecord::Base
  ...
  has_many :users, :through => :user_group_memberships, :class_name => 'PersistentUser'

  has_many :group_permissions, :class_name => 'GroupPermission'
  has_many :permissions, :through => :group_permissions, :class_name => 'Permission'

  has_many :group_sections, :class_name => 'GroupSection'
  has_many :sections, :through => :group_sections, :class_name => 'Section'
  ...
  scope :public, -> { where(["#{GroupType.table_name}.cms_access = ?", false]).includes(:group_type).references(:group_type) }
  scope :cms_access, -> { where(["#{GroupType.table_name}.cms_access = ?", true]).includes(:group_type).references(:group_type) }
