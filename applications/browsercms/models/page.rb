class Page < ActiveRecord::Base
  ...
  class << self
    ...
    def connected_to(b)
      ...
      if ver
        query = where(["#{Connector.table_name}.connectable_id = ? and #{Connector.table_name}.connectable_type = ? and #{Connector.table_name}.connectable_version = ?", obj.id, obj.class.base_class.name, ver])
      else
        query = where(["#{Connector.table_name}.connectable_id = ? and #{Connector.table_name}.connectable_type = ?", obj.id, obj.class.base_class.name])
      end
  ...
  scope :currently_connected_to, lambda { |obj|
    connectors_table = Connector.table_name

    ver = obj.class.versioned? ? obj.version : nil
    if ver
      where(["#{connectors_table}.connectable_id = ? and #{connectors_table}.connectable_type = ? and #{connectors_table}.connectable_version = ? and #{connectors_table}.page_version = #{Page.table_name}.version", obj.id, obj.class.base_class.name, ver]).includes(:connectors).references(:connectors)
    else
      where(["#{connectors_table}.connectable_id = ? and #{connectors_table}.connectable_type = ? and #{connectors_table}.page_version = #{Page.table_name}.version", obj.id, obj.class.base_class.name]).includes(:connectors).references(:connectors)
    end
  }
  ...
  # @return [Page::Version] The version of the page as of the current Draft.
  # @raises [Errors::ContentNotFound] if no record could be found.
  ...
  # @return [Page] The page if found
  # @rais [Errors::ContentNotFound] If no published page was found with the given path.
  def self.find_live(path)
    result = find_live_by_path(path)
    unless result
      raise Errors::ContentNotFound
    end
    result
  end
  ...
  def copy_connectors(options={})
    ...
    c_found = connectors.for_page_version(options[:from_version_number]).order("#{Connector.table_name}.container, #{Connector.table_name}.position").to_a
