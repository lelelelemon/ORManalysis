  ...
  class PersistentUser < ActiveRecord::Base
    ...
    has_many :user_group_memberships, :class_name => 'UserGroupMembership', foreign_key: :user_id
    has_many :groups, :through => :user_group_memberships, :class_name => 'Group', foreign_key: :user_id
    has_many :tasks, :foreign_key => "assigned_to_id", :class_name => 'Task'
    ...
    class << self
      ...
      #
      # @return [ActiveRelation<User>] A scope which will find users with the correct permissions.
    ...
    def permissions
      @permissions ||= Permission.where(["#{self.class.table_name}.id = ?", id]).includes({:groups => :users}).references(:users)
    end

    def viewable_sections
      @viewable_sections ||= Section.where(["#{self.class.table_name}.id = ?", id]).includes(:groups => :users).references(:users)
    end

    def modifiable_sections
      @modifiable_sections ||= Section.where(["#{self.class.table_name}.id = ? and #{GroupType.table_name}.cms_access = ?", id, true]).includes(:groups => [:group_type, :users]).references(:users, :groups)
    end
