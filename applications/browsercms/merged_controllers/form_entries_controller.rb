      ...
      if @entry.save
        ...
        unless @form.notification_email.blank?
          EmailMessage.create!(
              ...
              #{EmailMessage.absolute_cms_url(cms.form_entry_path(@entry)) }"
          )
        end
      else
        show_content_as_page(@form)
...
ruby_code_from_view.ruby_code_from_view do |rb_from_view|
 content_for :main , @form.confirmation_text.html_safe 
    ...
    def index
      ...
      @search_filter = SearchFilter.build(params[:search_filter], FormEntry)
        
      @blocks = FormEntry.where(form_id: params[:id]).search(@search_filter.term).paginate({page: params[:page], order: params[:order]})
...
ruby_code_from_view.ruby_code_from_view do |rb_from_view|
 ...
 f.input field.name, field.options(entry: model) 
    ...
    def update
      @entry = FormEntry.find(params[:id]).enable_validations
...
ruby_code_from_view.ruby_code_from_view do |rb_from_view|
 ...
 @entry.data_columns[field.name] 
    ...
    def new
      @entry = FormEntry.for(Form.find(params[:form_id]))
ruby_code_from_view.ruby_code_from_view do |rb_from_view|
 ...
 f.input field.name, field.options(entry: model) 
    ...
    class FauxContentType < ContentType
      def initialize(form)
        @form = form
        self.name = 'FormEntry'
      end
