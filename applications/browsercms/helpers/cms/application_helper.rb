  module FormTagHelper
    ...
    def content_block_form_for(object, *args, &block)
      ...
      simple_form_for(engine_aware_path(object), *(args << options.merge(builder: FormBuilder::ContentBlockFormBuilder, wrapper: 'browsercms')), &block)
    end
    ...
    def cms_form_for(*args, &block)
      options = args.extract_options!
      options.merge!(:builder => FormBuilder::ContentBlockFormBuilder)
      form_for(*(args + [options]), &block)
    end

    def forecasting_a_new_section?(form_object)
      Section.with_path(form_object.object.class.path).first.nil?
    end
