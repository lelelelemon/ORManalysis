...
#             which may be different than the 'front end' user login page. (Controller handles that differently)
#
#
  ...
  module Authentication
    module Controller
      ...
      module ClassMethods
        ...
        # @raise [Errors::AccessDenied] If the current_user doesn't have ANY of the given permissions.
        #
        # Example:
        ...
        def check_permissions(*perms)
          ...
          before_filter(opts) do |controller|
            raise Errors::AccessDenied unless controller.send(:current_user).able_to?(*perms)
          end
        end
      end
      ...
      def current_user
        @current_user ||= begin
          PersistentUser.current = current_cms_user || User.guest
        end
      end
      ...
      def logout_keeping_session!
        ...
        PersistentUser.current.forget_me if User.current.is_a? User
        PersistentUser.current = false # not logged in, and don't do it for me
