...
module Cms
  module ContentRenderingSupport
    ...
    def self.included(base)
      base.send :include, ErrorHandling
      ...
      base.rescue_from Errors::ContentNotFound, :with => :handle_not_found_on_page
      base.rescue_from Errors::DraftNotFound, :with => :handle_draft_not_found
      base.rescue_from Errors::AccessDenied, :with => :handle_access_denied_on_page
    ...
    def handle_not_found_on_page(exception)
      ...
      handle_error_with_cms_page(ErrorPages::NOT_FOUND_PATH, exception, :not_found)
    end

    def handle_access_denied_on_page(exception)
      logger.warn "Access denied for user '#{current_user.login}': Returning the 403 page."
      handle_error_with_cms_page(ErrorPages::FORBIDDEN_PATH, exception, :forbidden)
    end

    def handle_server_error_on_page(exception)
      logger.error "An Unexpected exception occurred: #{exception.message}\n"
      logger.error "#{exception.backtrace.join("\n")}\n"
      handle_error_with_cms_page(ErrorPages::SERVER_ERROR_PATH, exception, :internal_server_error)
    end
