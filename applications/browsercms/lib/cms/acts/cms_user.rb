  ...
  module Acts
    ...
    #     acts_as_cms_user :groups => [Group.find_by_code('admin')]
    #   end
    #
    ...
    User
      ...
      module MacroMethods
        def acts_as_cms_user(options = {})
          ...
          # instance method on the person.  Defaults to Group.guest
          define_method "cms_groups" do
            ...
            @cms_groups ||= begin
              specified = Array(fetch_group_list.call).select { |x| x.kind_of? Group }
              specified << Group.guest if specified.empty?
              specified
            end
        ...
        module InstanceMethods
          ...
          # The following came from GuestUser or User
          ...
          def viewable_sections
            @viewable_sections ||= Section.includes(:groups).where(["#{Group.table_name}.id  in (?)", cms_groups.collect(&:id)])
          end

          def permissions
            @permissions ||= Permission.includes(:groups).where(["#{Group.table_name}.id  in (?)", cms_groups.collect(&:id)])
          end
