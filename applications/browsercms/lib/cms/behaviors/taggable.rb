    ...
    module Taggable
      ...
      module MacroMethods
        ...
        def is_taggable(options={})
          ...
          has_many :tags, -> { order("#{Tag.table_name}.name") }, :class_name => 'Tag', :through => :taggings
      ...
      module ClassMethods
        def tagged_with(t)
          where(["#{Tag.table_name}.name = ?", t]).includes(:taggings => :tag).references(:tags)
        end
      ...
      module InstanceMethods
        ...
        def save_tags
          tag_list_tags = tag_list.to_s.split(self.class.tag_separator).map { |t| Tag.find_or_create_by(name: t.strip) }
          taggings.each do |tg|
