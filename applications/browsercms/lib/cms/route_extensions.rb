  ...
  def guess_model_class(content_block_name)
    ...
    begin
      namespaced_model_name = "#{Module.current_namespace}::#{content_name}"
  ...
  def add_routes_for_addressable_content_blocks
    classes = Concerns::Addressable.classes_that_require_custom_routes
  ...
  def inline_route(denamespaced_controller, inline_route_name, klass)
    begin
      ...
      # This avoids the error when PageRoute.reload_routes is called.
  ...
  def add_page_routes_defined_in_database
    if PageRoute.can_be_loaded?
      PageRoute.order("#{PageRoute.table_name}.name").each do |r|
